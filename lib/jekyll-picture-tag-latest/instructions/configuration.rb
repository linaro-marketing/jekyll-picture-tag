module PictureTag
  module Instructions
    ...
    class Configuration
      ...
      # Source dir is the jekyll-picture-tag-latest source directory.
      def source_dir
        File.join PictureTag.site.source, self['picture']['source']
      end

      # site.dest is the master jekyll destination directory
      # source_dest is the jekyll-picture-tag-latest destination directory. (generated
      ...
      def build_config
        YAML.safe_load(
          ...
          File.read(
            File.join(ROOT_PATH, 'jekyll-picture-tag-latest/defaults/global.yml')
          )
        ).merge(
